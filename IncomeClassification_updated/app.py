...
# Load the one-hot encoder. (This pickle
# file was originally saved by the code that trained 
# the model. See mlmodel.py)
enc = load(open('enc_le.pkl', 'rb'))
...
def input(input_row1):
    ...
    # Transform the OneHotEncoder using the categorical variable list
    ...
    #answer=prediction_labels[predict[0]]
    print(predict)
    #print(f'Prediction 1 is:{answer}')
    #return answer
    if predict[0]==0:
        print("Less than 50,000K")
        a=True
    elif predict[0]==1:
        print("Greater than 50,000K")
        a=False
    return a
